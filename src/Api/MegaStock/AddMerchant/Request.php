...
namespace grandmasterx\WebMoney\Api\MegaStock\AddMerchant;

use grandmasterx\WebMoney\Signer;
use grandmasterx\WebMoney\Api\MegaStock;
use grandmasterx\WebMoney\Exception\ApiException;
use grandmasterx\WebMoney\Request\RequestValidator;
use grandmasterx\WebMoney\Api\MegaStock\Request as MegastockRequest;
    ...
    protected function getValidationRules()
    {
        return [
            RequestValidator::TYPE_REQUIRED => [
                'integratorId', 'integratorWmid', 'url', 'categoryId',
                'aboutLanguage', 'aboutName', 'aboutDescription',
                'merchantNameInComment', 'geoBindings'
            ],
            RequestValidator::TYPE_ARRAY => ['about', 'geoBindings'],
            RequestValidator::TYPE_DEPEND_REQUIRED => [
                'beneficiaryLegalName' => ['loginType' => [self::LOGIN_TYPE_KEEPER]],
                'beneficiaryLegalOgrn' => ['loginType' => [self::LOGIN_TYPE_KEEPER]],
                'beneficiaryWmid' => ['loginType' => [self::LOGIN_TYPE_PROCESSING]]
            ],
            RequestValidator::TYPE_CONDITIONAL => [
                'beneficiaryType' => [
                    [
                        'value' => self::BENEFICIARY_TYPE_CONTRACT,
                        'conditional' => ['loginType' => self::LOGIN_TYPE_KEEPER]
                    ],
                    [
                        'value' => self::BENEFICIARY_TYPE_WEBMONEY,
                        'conditional' => ['loginType' => self::LOGIN_TYPE_PROCESSING]
                    ]
                ]
            ]
        ];
    ...
    public function sign(Signer $requestSigner = null)
    {
        $signString = implode(
            ...
            [
                ...
                $this->integratorId,
                $this->integratorWmid,
                mb_convert_encoding($this->merchantNameInComment, 'Windows-1251', 'UTF-8'),
                $this->categoryId
            ]
        );
